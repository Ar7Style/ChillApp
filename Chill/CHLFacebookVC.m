...
- (void)startSearchingFacebookFriends {
    ...
    //TODO: implement facebook fetching with callback
}
...
- (void)sendFacebookIDToBackend {
    ...
    NSError *error = nil;
    NSString *facebookID = [[FBSDKAccessToken currentAccessToken] appID];
...
- (void)fetchFacebookFriendsInChill {
    ...
    NSString* url = [NSString stringWithFormat:@"http://api.iamchill.co/v2/social/twitter/id_user/%@/id_contacts_facebook/%@", [[NSUserDefaults standardUserDefaults] valueForKey:@"id_user"], str_facebook_IDs];
